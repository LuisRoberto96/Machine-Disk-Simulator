...
void atualiza_arquivo(Linha*cache,int tam){
	...
	FILE* DISCO;
		DISCO= fopen("disco.dat","w");
		fwrite(&aux,sizeof(Linha),1,DISCO);
		fclose(DISCO);
}

/*void atualiza_arquivo(Linha umaLinha){
	Linha*aux = new Linha[TAM_DISCO];
	if(umaLinha.atualiza==1){
			aux[0].endBloco=umaLinha.endBloco;
			aux[0].atualiza=0;
			aux[0].cachehit=0;
			for(int j=0;j<4;j++)
				aux[0].palavras[j]=umaLinha.palavras[j];
	}
	}
